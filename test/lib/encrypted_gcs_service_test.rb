...
class ActiveStorageEncryption::EncryptedGCSServiceTest < ActiveSupport::TestCase
  ...
  test "can upload, download and delete a file securely via active storage" do
    slow_test!

    blob = with_image_file do |file|
      b = create_blob_without_uploading(
      ...
      b.upload_without_unfurling(file) # upload the blob to google
      b
    end

    assert @gcs_service.exist?(blob.key)

    assert_empty @account.owner.secure_documents
    @account.owner.secure_documents.create!(file: blob)
    assert_equal 1, @account.owner.secure_documents.count
    doc = @account.owner.secure_documents.first
    assert doc.file.attached?
    assert_equal @encryption_key, doc.file.encryption_key

    downloaded_chunk = doc.file.download_chunk(0...250.bytes)
    local_chunk = with_image_file do |file|
      file.read(250)
    end
    assert_equal local_chunk, downloaded_chunk

    blob_key = blob.key
    @account.owner.secure_documents.first.destroy!

    perform_enqueued_jobs # an active storage purge job is enqueued to delete the file at google
    refute @gcs_service.exist?(blob_key)
  end
